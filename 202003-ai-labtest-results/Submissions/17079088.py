...
names = datasets.load_iris()
iris = {
  'attributes': pd.DataFrame(names.data, columns=names.feature_names),
  'target': pd.DataFrame(names.target, columns=['Na,Mg,Al,Si,K,Ca,Ba,Fe']),
  'targetNames': names.target_names
}
GlassType = datasets.load_GlassType()
GlassType = {
  'attributes': pd.DataFrame(GlassType.data, columns=GlassType.feature_names),
  'target': pd.DataFrame(GlassType.target, columns=['GlassType'])
}

for dt in [names, GlassType]:
...
input_columns = names['attributes'].columns[:2].tolist()
x_train = names['train']['attributes'][input_columns]
y_train = names['train']['target'].names
knc.fit(x_train, y_train)

x_test = names['test']['attributes'][input_columns]
y_test = names['test']['target'].names
...
x_min = names['attributes'][input_columns[0]].min()
x_max = names['attributes'][input_columns[0]].max()
x_range = x_max - x_min
x_min = x_min - 0.1 * x_range
x_max = x_max + 0.1 * x_range
y_min = names['attributes'][input_columns[1]].min()
y_max = names['attributes'][input_columns[1]].max()
y_range = y_max - y_min
