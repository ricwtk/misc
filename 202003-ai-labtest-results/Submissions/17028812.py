...
#Na, Mg, Al, Si, K, Ca, Ba, and Fe (i.e. all columns except Glass type) as the input features.
data = {
    'attributes': pd.DataFrame(data[['Na','Mg','Al','Si','K', 'Ca', 'Ba', 'Fe']]),
...
for i in range(3,101):
    ...
    print("accuray for k =" + str(i) + " is  " + str(knc.score(x_test,y_test)))                             
    
   
 
plt.figure()                                                                        
plt.plot(kncList, accuracy, color='black')
plt.title('Accuracy for KNC from 3 to 100')