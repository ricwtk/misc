...
data = {
    'attributes' : pd.DataFrame(columns =[["Id"],["RI"],["Na"],["Mg"],["Al"],["Si"],["K"],["Ca"],["Ba"],["Fe"]])
    ,'target' : pd.DataFrame(columns =['Glass type'])
    }

for dt in [data]:
     x_train, x_test, y_train, y_test = train_test_split(dt['attributes'], dt['target'], test_size=0.3)
     dt['train'] = {
         'attributes': x_train,
         'target': y_train
  }
...
knc = KNeighborsClassifier(49)

input_columns = data['attributes'].columns[:10].tolist()
x_train = data['train']['attributes'][input_columns]
y_train = data['train']['target'][input_columns]
knc.fit(x_train, y_train)

x_test = data['test']['attributes'][input_columns]
y_test = data['test']['target'][input_columns]
...
cm_dark = ListedColormap(colormap.colors[::10])
